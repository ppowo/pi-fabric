...
`agents.tell` enqueues mail for a persistent actor; for a **running one-shot subagent** use `agents.steer({ id, message })`, which is delivered between the child's turns and preserves its context. Any Fabric-equipped Pi participant can message a known Main/agent/actor target: `agents.steer` routes locally when possible and otherwise publishes an exact-id `fabric.steer` mesh event the owner relays. Ordinary non-recursive and Claude agents can receive this delivery but cannot initiate Fabric calls. Use `agents.steer` to redirect a teammate's running worker without discarding its work.
